Title: Game Tests | NeoForged docs

URL Source: https://docs.neoforged.net/docs/1.21.1/misc/gametest
...
## Creating a Game Test[​](https://docs.neoforged.net/docs/1.21.1/misc/gametest#creating-a-game-test "Direct link to Creating a Game Test")
...
### The Test Method[​](https://docs.neoforged.net/docs/1.21.1/misc/gametest#the-test-method "Direct link to The Test Method")
...
`public class ExampleGameTests {    @GameTest    public static void exampleTest(GameTestHelper helper) {        // Do stuff    }}`

The `@GameTest` annotation also contains members which configure how the game test should run.

`// In some class@GameTest(    setupTicks = 20L, // The test spends 20 ticks to set up for execution    required = false // The failure is logged but does not affect the execution of the batch)public static void exampleConfiguredTest(GameTestHelper helper) {    // Do stuff}`

#### Relative Positioning[​](https://docs.neoforged.net/docs/1.21.1/misc/gametest#relative-positioning "Direct link to Relative Positioning")
...
#### Successful Completion[​](https://docs.neoforged.net/docs/1.21.1/misc/gametest#successful-completion "Direct link to Successful Completion")
...
#### Scheduling Actions[​](https://docs.neoforged.net/docs/1.21.1/misc/gametest#scheduling-actions "Direct link to Scheduling Actions")
...
#### Assertions[​](https://docs.neoforged.net/docs/1.21.1/misc/gametest#assertions "Direct link to Assertions")
...
### Generated Test Methods[​](https://docs.neoforged.net/docs/1.21.1/misc/gametest#generated-test-methods "Direct link to Generated Test Methods")
...
`public class ExampleGameTests {    @GameTestGenerator    public static Collection<TestFunction> exampleTests() {        // Return a collection of TestFunctions    }}`

#### TestFunction[​](https://docs.neoforged.net/docs/1.21.1/misc/gametest#testfunction "Direct link to TestFunction")
...
### Batching[​](https://docs.neoforged.net/docs/1.21.1/misc/gametest#batching "Direct link to Batching")
...
`public class ExampleGameTests {    @BeforeBatch(batch = "firstBatch")    public static void beforeTest(ServerLevel level) {        // Perform setup    }    @GameTest(batch = "firstBatch")    public static void exampleTest2(GameTestHelper helper) {        // Do stuff    }}`

## Registering a Game Test[​](https://docs.neoforged.net/docs/1.21.1/misc/gametest#registering-a-game-test "Direct link to Registering a Game Test")
...
### GameTestHolder[​](https://docs.neoforged.net/docs/1.21.1/misc/gametest#gametestholder "Direct link to GameTestHolder")
...
`@GameTestHolder(MODID)public class ExampleGameTests {    // ...}`

### RegisterGameTestsEvent[​](https://docs.neoforged.net/docs/1.21.1/misc/gametest#registergametestsevent "Direct link to RegisterGameTestsEvent")

`RegisterGameTestsEvent` can also register either classes or methods using `#register`. The event listener must be [added](https://docs.neoforged.net/docs/1.21.1/concepts/events#registering-an-event-handler) to the mod event bus. Test methods registered this way must supply their mod id to `GameTest#templateNamespace` on every method annotated with `@GameTest`.

`// In some event handler class@SubscribeEvent  // on the mod event buspublic static void registerTests(RegisterGameTestsEvent event) {    event.register(ExampleGameTests.class);}// In ExampleGameTests@GameTest(templateNamespace = MODID)public static void exampleTest3(GameTestHelper helper) {    // Perform setup}`

note

The value supplied to `GameTestHolder#value` and `GameTest#templateNamespace` can be different from the current mod id. The configuration within the [buildscript](https://docs.neoforged.net/docs/1.21.1/misc/gametest#enabling-other-namespaces) would need to be changed.

## Structure Templates[​](https://docs.neoforged.net/docs/1.21.1/misc/gametest#structure-templates "Direct link to Structure Templates")
...
## Buildscript Configurations[​](https://docs.neoforged.net/docs/1.21.1/misc/gametest#buildscript-configurations "Direct link to Buildscript Configurations")
...
### Enabling Other Namespaces[​](https://docs.neoforged.net/docs/1.21.1/misc/gametest#enabling-other-namespaces "Direct link to Enabling Other Namespaces")

If the buildscript was [setup as recommended](https://docs.neoforged.net/docs/1.21.1/gettingstarted/#simple-buildgradle-customizations), then only Game Tests under the current mod id would be enabled. To enable other namespaces to load Game Tests from, a run configuration must set the property `neoforge.enabledGameTestNamespaces` to a string specifying each namespace separated by a comma. If the property is empty or not set, then all namespaces will be loaded.

`// Inside a run configurationproperty 'neoforge.enabledGameTestNamespaces', 'modid1,modid2,modid3'`

caution

There must be no spaces in-between namespaces; otherwise, the namespace will not be loaded correctly.

### Game Test Server Run Configuration[​](https://docs.neoforged.net/docs/1.21.1/misc/gametest#game-test-server-run-configuration "Direct link to Game Test Server Run Configuration")
...
Important infromation on NeoGradle

### Enabling Game Tests in Other Run Configurations[​](https://docs.neoforged.net/docs/1.21.1/misc/gametest#enabling-game-tests-in-other-run-configurations "Direct link to Enabling Game Tests in Other Run Configurations")
...
`// Inside a run configurationproperty 'neoforge.enableGameTest', 'true'`