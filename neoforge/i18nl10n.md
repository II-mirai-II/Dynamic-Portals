Title: I18n and L10n | NeoForged docs

URL Source: https://docs.neoforged.net/docs/1.21.1/resources/client/i18n
...
## `Component`s[​](https://docs.neoforged.net/docs/1.21.1/resources/client/i18n#components "Direct link to components")
...
| `nbt` | Creates a component representing the [NBT](https://docs.neoforged.net/docs/1.21.1/datastorage/nbt) at the given path. |
| `score` | Creates a component containing a scoreboard objective value. |
| `selector` | Creates a component containing a list of entity names for a given [entity selector](https://minecraft.wiki/w/Target_selectors). |
...
### `MutableComponent`[​](https://docs.neoforged.net/docs/1.21.1/resources/client/i18n#mutablecomponent "Direct link to mutablecomponent")

All constructed components are generally `MutableComponent`s. A `MutableComponent` provides methods to add new component siblings that are appended to the end of this component and set the [style](https://docs.neoforged.net/docs/1.21.1/resources/client/i18n#text-formatting) of the text. Constructing or modifying a component should always refer to `MutableComponent`s.

### Text Formatting[​](https://docs.neoforged.net/docs/1.21.1/resources/client/i18n#text-formatting "Direct link to Text Formatting")
...
`{    "translation.key.1": "Translation 1",    "translation.key.2": "Translation 2"}`

### Translation Keys[​](https://docs.neoforged.net/docs/1.21.1/resources/client/i18n#translation-keys "Direct link to Translation Keys")
...
Some places in Minecraft offer you helper methods to get a translation keys. For example, both blocks and items provide `#getDescriptionId` methods. These can not only be queried, but also overridden if needed. A common use case are items that have different names depending on their [NBT](https://docs.neoforged.net/docs/1.21.1/datastorage/nbt) value. These will usually override the variant of `#getDescriptionId` that has an [`ItemStack`](https://docs.neoforged.net/docs/1.21.1/items/#itemstacks) parameter, and return different values based on the stack's NBT. Another common use case are `BlockItem`s, which override the method to use the associated block's translation key instead.

tip

The only purpose of translation keys is for localization. Do not use them for game logic, that's what [registry names](https://docs.neoforged.net/docs/1.21.1/concepts/registries) are for.

### Translating Mod Metadata[​](https://docs.neoforged.net/docs/1.21.1/resources/client/i18n#translating-mod-metadata "Direct link to Translating Mod Metadata")

Starting with NeoForge 20.4.179, translation files can override certain parts of [mod info](https://docs.neoforged.net/docs/1.21.1/gettingstarted/modfiles#modstoml) using the following keys (where `modid` is to be replaced with the actual mod id):
...
### Datagen[​](https://docs.neoforged.net/docs/1.21.1/resources/client/i18n#datagen "Direct link to Datagen")

Language files can be [datagenned](https://docs.neoforged.net/docs/1.21.1/resources/#data-generation). To do so, extend the `LanguageProvider` class and add your translations in the `addTranslations()` method:

`public class MyLanguageProvider extends LanguageProvider {    public MyLanguageProvider(PackOutput output) {        super(            // Provided by the GatherDataEvent.            output,            // Your mod id.            "examplemod",            // The locale to use. You may use multiple language providers for different locales.            "en_us"        );    }        @Override    protected void addTranslations() {        // Adds a translation with the given key and the given value.        add("translation.key.1", "Translation 1");                // Helpers are available for various common object types. Every helper has two variants: an add() variant        // for the object itself, and an addTypeHere() variant that accepts a supplier for the object.        // The different names for the supplier variants are required due to generic type erasure.        // All following examples assume the existence of the values as suppliers of the needed type.        // Adds a block translation.        add(MyBlocks.EXAMPLE_BLOCK.get(), "Example Block");        addBlock(MyBlocks.EXAMPLE_BLOCK, "Example Block");        // Adds an item translation.        add(MyItems.EXAMPLE_ITEM.get(), "Example Item");        addItem(MyItems.EXAMPLE_ITEM, "Example Item");        // Adds an item stack translation. This is mainly for items that have NBT-specific names.        add(MyItems.EXAMPLE_ITEM_STACK.get(), "Example Item");        addItemStack(MyItems.EXAMPLE_ITEM_STACK, "Example Item");        // Adds an entity type translation.        add(MyEntityTypes.EXAMPLE_ENTITY_TYPE.get(), "Example Entity");        addEntityType(MyEntityTypes.EXAMPLE_ENTITY_TYPE, "Example Entity");        // Adds an enchantment translation.        add(MyEnchantments.EXAMPLE_ENCHANTMENT.get(), "Example Enchantment");        addEnchantment(MyEnchantments.EXAMPLE_ENCHANTMENT, "Example Enchantment");        // Adds a mob effect translation.        add(MyMobEffects.EXAMPLE_MOB_EFFECT.get(), "Example Effect");        addEffect(MyMobEffects.EXAMPLE_MOB_EFFECT, "Example Effect");    }}`
