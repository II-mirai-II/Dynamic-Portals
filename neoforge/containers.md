Title: Containers | NeoForged docs

URL Source: https://docs.neoforged.net/docs/1.21.1/inventories/container

Markdown Content:
Many systems in Minecraft, such as [block entities](https://docs.neoforged.net/docs/1.21.1/blockentities/) or entities, store items of some kind. To store items on something, Minecraft uses `Container` implementations.
...
Due to this, `Container`s can not only be implemented on block entities, but any other class as well. Notable examples include entity inventories, as well as common modded [items](https://docs.neoforged.net/docs/1.21.1/items/) such as backpacks.
...
## Basic Container Implementation[​](https://docs.neoforged.net/docs/1.21.1/inventories/container#basic-container-implementation "Direct link to Basic Container Implementation")
...
### `BaseContainerBlockEntity`[​](https://docs.neoforged.net/docs/1.21.1/inventories/container#basecontainerblockentity "Direct link to basecontainerblockentity")
...
*   `Nameable` defines a few methods related to setting (custom) names and, aside from many block entities, is implemented by classes such as `Entity`. This uses the [`Component` system](https://docs.neoforged.net/docs/1.21.1/resources/client/i18n#components).
*   `MenuProvider`, on the other hand, defines the `#createMenu` method, which allows an [`AbstractContainerMenu`](https://docs.neoforged.net/docs/1.21.1/gui/menus) to be constructed from the container. This means that using this class is not desirable if you want a container without an associated GUI, for example in jukeboxes.
...
`public class MyBlockEntity extends BaseContainerBlockEntity {    // The container size. This can of course be any value you want.    public static final int SIZE = 9;    // Our item stack list. This is not final due to #setItems existing.    private NonNullList<ItemStack> items = NonNullList.withSize(SIZE, ItemStack.EMPTY);    // The constructor, like before.    public MyBlockEntity(BlockPos pos, BlockState blockState) {        super(MY_BLOCK_ENTITY.get(), pos, blockState);    }    // The container size, like before.    @Override    public int getContainerSize() {        return SIZE;    }    // The getter for our item stack list.    @Override    protected NonNullList<ItemStack> getItems() {        return items;    }    // The setter for our item stack list.    @Override    protected void setItems(NonNullList<ItemStack> items) {        this.items = items;    }    // The display name of the menu. Don't forget to add a translation!    @Override    protected Component getDefaultName() {        return Component.translatable("container.examplemod.myblockentity");    }    // The menu to create from this container. See below for what to return here.    @Override    protected AbstractContainerMenu createMenu(int containerId, Inventory inventory) {        return null;    }}`
...
### `WorldlyContainer`[​](https://docs.neoforged.net/docs/1.21.1/inventories/container#worldlycontainer "Direct link to worldlycontainer")
...
`if (blockEntity instanceof Container container) {    // do something with the container}`

The container can then use the methods we mentioned before, for example:

`// Get the first item in the container.ItemStack stack = container.getItem(0);// Set the first item in the container to dirt.container.setItem(0, new ItemStack(Items.DIRT));// Removes a quantity of (up to) 16 from the third slot.container.removeItem(2, 16);`
...
## `Container`s on `Entity`s[​](https://docs.neoforged.net/docs/1.21.1/inventories/container#containers-on-entitys "Direct link to containers-on-entitys")
...
### `Container`s on `Mob`s[​](https://docs.neoforged.net/docs/1.21.1/inventories/container#containers-on-mobs "Direct link to containers-on-mobs")
...
`// Get the item stack in the HEAD (helmet) slot.ItemStack helmet = mob.getItemBySlot(EquipmentSlot.HEAD);// Put bedrock into the mob's FEET (boots) slot.mob.setItemSlot(EquipmentSlot.FEET, new ItemStack(Items.BEDROCK));// Enable that bedrock to always drop if the mob is killed.mob.setDropChance(EquipmentSlot.FEET, 1f);`

### `InventoryCarrier`[​](https://docs.neoforged.net/docs/1.21.1/inventories/container#inventorycarrier "Direct link to inventorycarrier")
...
### `Container`s on `Player`s (Player Inventory)[​](https://docs.neoforged.net/docs/1.21.1/inventories/container#containers-on-players-player-inventory "Direct link to containers-on-players-player-inventory")
