Title: Sounds | NeoForged docs

URL Source: https://docs.neoforged.net/docs/1.21.1/resources/client/sounds
...
## Terminology[​](https://docs.neoforged.net/docs/1.21.1/resources/client/sounds#terminology "Direct link to Terminology")
...
*   **Sound definition**: A mapping of a sound event to one or multiple sound objects, plus some optional metadata. Sound definitions are located in a namespace's [`sounds.json` file](https://docs.neoforged.net/docs/1.21.1/resources/client/sounds#soundsjson).
...
Due to the implementation of OpenAL (Minecraft's audio library), for your sound to have attenuation - that is, for it to get quieter and louder depending on the player's distance to it -, your sound file must be mono (single channel). Stereo (multichannel) sound files will not be subject to attenuation and always play at the player's location, making them ideal for ambient sounds and background music. See also [MC-146721](https://bugs.mojang.com/browse/MC-146721).

## Creating `SoundEvent`s[​](https://docs.neoforged.net/docs/1.21.1/resources/client/sounds#creating-soundevents "Direct link to creating-soundevents")

`SoundEvent`s are [registered objects](https://docs.neoforged.net/docs/1.21.1/concepts/registries), meaning that they must be registered to the game through a `DeferredRegister` and be singletons:

`public class MySoundsClass {    // Assuming that your mod id is examplemod    public static final DeferredRegister<SoundEvent> SOUND_EVENTS =            DeferredRegister.create(BuiltInRegistries.SOUND_EVENT, "examplemod");        // All vanilla sounds use variable range events.    public static final DeferredHolder<SoundEvent, SoundEvent> MY_SOUND = SOUND_EVENTS.register(            "my_sound", // must match the resource location on the next line            () -> SoundEvent.createVariableRangeEvent(ResourceLocation.fromNamespaceAndPath("examplemod", "my_sound"))    );        // There is a currently unused method to register fixed range (= non-attenuating) events as well:    public static final DeferredHolder<SoundEvent, SoundEvent> MY_FIXED_SOUND = SOUND_EVENTS.register("my_fixed_sound",            // 16 is the default range of sounds. Be aware that due to OpenAL limitations,            // values above 16 have no effect and will be capped to 16.            () -> SoundEvent.createFixedRangeEvent(ResourceLocation.fromNamespaceAndPath("examplemod", "my_fixed_sound"), 16)    );}`

Of course, don't forget to add your registry to the [mod event bus](https://docs.neoforged.net/docs/1.21.1/concepts/events#event-buses) in the [mod constructor](https://docs.neoforged.net/docs/1.21.1/gettingstarted/modfiles#javafml-and-mod):

`public ExampleMod(IEventBus modBus) {    MySoundsClass.SOUND_EVENTS.register(modBus);    // other things here}`

And voilà, you have a sound event!

## `sounds.json`[​](https://docs.neoforged.net/docs/1.21.1/resources/client/sounds#soundsjson "Direct link to soundsjson")

_See also: [sounds.json](https://minecraft.wiki/w/Sounds.json) on the [Minecraft Wiki](https://minecraft.wiki/)_
...
`{    "sound_1": {        "sounds": [            "sound_1"        ]    },    "sound_2": {        "replace": true,        "sounds": [            "sound_2"        ]    },    "sound_3": {        "sounds": [            "sound_3"        ]    },    "sound_4": {        "replace": true,        "sounds": [            "sound_4"        ]    }}`

`sounds.json` in RP2:

`{    "sound_1": {        "sounds": [            "sound_5"        ]    },    "sound_2": {        "sounds": [            "sound_6"        ]    },    "sound_3": {        "replace": true,        "sounds": [            "sound_7"        ]    },    "sound_4": {        "replace": true,        "sounds": [            "sound_8"        ]    }}`
...
`{    "sound_1": {        // replace false and false: add from lower pack, then from upper pack        "sounds": [            "sound_5",            "sound_1"        ]    },    "sound_2": {        // replace true in upper pack and false in lower pack: add from upper pack only        "sounds": [            "sound_2"        ]    },    "sound_3": {        // replace false in upper pack and true in lower pack: add from lower pack, then from upper pack        // Would still discard values from a third resource pack sitting below RP2        "sounds": [            "sound_7",            "sound_3"        ]    },    "sound_4": {        // replace true and true: add from upper pack only        "sounds": [            "sound_8"        ]    }}`

## Playing Sounds[​](https://docs.neoforged.net/docs/1.21.1/resources/client/sounds#playing-sounds "Direct link to Playing Sounds")

Minecraft offers various methods to play sounds, and it is sometimes unclear which one should be used. All methods accept a `SoundEvent`, which can either be your own or a vanilla one (vanilla sound events are found in the `SoundEvents` class). For the following method descriptions, client and server refer to the [logical client and logical server](https://docs.neoforged.net/docs/1.21.1/concepts/sides#the-logical-side), respectively.

### `Level`[​](https://docs.neoforged.net/docs/1.21.1/resources/client/sounds#level "Direct link to level")
...
### `ClientLevel`[​](https://docs.neoforged.net/docs/1.21.1/resources/client/sounds#clientlevel "Direct link to clientlevel")
...
### `Entity`[​](https://docs.neoforged.net/docs/1.21.1/resources/client/sounds#entity "Direct link to entity")
...
### `Player`[​](https://docs.neoforged.net/docs/1.21.1/resources/client/sounds#player "Direct link to player")
