Title: Registries | NeoForged docs

URL Source: https://docs.neoforged.net/docs/1.21.1/concepts/registries

Markdown Content:
Registration is the process of taking the objects of a mod (such as [items](https://docs.neoforged.net/docs/1.21.1/items/), [blocks](https://docs.neoforged.net/docs/1.21.1/blocks/), entities, etc.) and making them known to the game. Registering things is important, as without registration the game will simply not know about these objects, which will cause unexplainable behaviors and crashes.
...
Every registered object has a unique name, called its registry name. The name is represented as a [`ResourceLocation`](https://docs.neoforged.net/docs/1.21.1/misc/resourcelocation). For example, the registry name of the dirt block is `minecraft:dirt`, and the registry name of the zombie is `minecraft:zombie`. Modded objects will of course not use the `minecraft` namespace; their mod id will be used instead.

## Vanilla vs. Modded[​](https://docs.neoforged.net/docs/1.21.1/concepts/registries#vanilla-vs-modded "Direct link to Vanilla vs. Modded")
...
## Methods for Registering[​](https://docs.neoforged.net/docs/1.21.1/concepts/registries#methods-for-registering "Direct link to Methods for Registering")
...
### `DeferredRegister`[​](https://docs.neoforged.net/docs/1.21.1/concepts/registries#deferredregister "Direct link to deferredregister")
...
We can then add our registry entries as static final fields (see [the article on Blocks](https://docs.neoforged.net/docs/1.21.1/blocks/) for what parameters to add in `new Block()`):
...
### `RegisterEvent`[​](https://docs.neoforged.net/docs/1.21.1/concepts/registries#registerevent "Direct link to registerevent")

`RegisterEvent` is the second way to register objects. This [event](https://docs.neoforged.net/docs/1.21.1/concepts/events) is fired for each registry, after the mod constructors (since those are where `DeferredRegister`s register their internal event handlers) and before the loading of configs. `RegisterEvent` is fired on the mod event bus.
...
## Querying Registries[​](https://docs.neoforged.net/docs/1.21.1/concepts/registries#querying-registries "Direct link to Querying Registries")
...
## Custom Registries[​](https://docs.neoforged.net/docs/1.21.1/concepts/registries#custom-registries "Direct link to Custom Registries")
...
Let's start by creating the [registry key](https://docs.neoforged.net/docs/1.21.1/misc/resourcelocation#resourcekeys) and the registry itself:
...
## Datapack Registries[​](https://docs.neoforged.net/docs/1.21.1/concepts/registries#datapack-registries "Direct link to Datapack Registries")

A datapack registry (also known as a dynamic registry or, after its main use case, worldgen registry) is a special kind of registry that loads data from [datapack](https://docs.neoforged.net/docs/1.21.1/resources/#data) JSONs (hence the name) at world load, instead of loading them when the game starts. Default datapack registries most notably include most worldgen registries, among a few others.

Datapack registries allow their contents to be specified in JSON files. This means that no code (other than [datagen](https://docs.neoforged.net/docs/1.21.1/concepts/registries#data-generation-for-datapack-registries) if you don't want to write the JSON files yourself) is necessary. Every datapack registry has a [`Codec`](https://docs.neoforged.net/docs/1.21.1/datastorage/codecs) associated with it, which is used for serialization, and each registry's id determines its datapack path:
...
### Custom Datapack Registries[​](https://docs.neoforged.net/docs/1.21.1/concepts/registries#custom-datapack-registries "Direct link to Custom Datapack Registries")

Custom datapack registries do not require a `Registry` to be constructed. Instead, they just need a registry key and at least one [`Codec`](https://docs.neoforged.net/docs/1.21.1/datastorage/codecs) to (de-)serialize its contents. Reiterating on the spells example from before, registering our spell registry as a datapack registry looks something like this:
...
### Data Generation for Datapack Registries[​](https://docs.neoforged.net/docs/1.21.1/concepts/registries#data-generation-for-datapack-registries "Direct link to Data Generation for Datapack Registries")

Since writing all the JSON files by hand is both tedious and error-prone, NeoForge provides a [data provider](https://docs.neoforged.net/docs/1.21.1/resources/#data-generation) to generate the JSON files for you. This works for both built-in and your own datapack registries.
