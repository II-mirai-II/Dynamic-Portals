Title: Menus | NeoForged docs

URL Source: https://docs.neoforged.net/docs/1.21.1/gui/menus/
...
## `MenuType`[​](https://docs.neoforged.net/docs/1.21.1/gui/menus/#menutype "Direct link to menutype")

Menus are created and removed dynamically and as such are not registry objects. As such, another factory object is registered instead to easily create and refer to the _type_ of the menu. For a menu, these are `MenuType`s.

`MenuType`s must be [registered](https://docs.neoforged.net/docs/1.21.1/concepts/registries#methods-for-registering).

### `MenuSupplier`[​](https://docs.neoforged.net/docs/1.21.1/gui/menus/#menusupplier "Direct link to menusupplier")

A `MenuType` is created by passing in a `MenuSupplier` and a `FeatureFlagSet` to its constructor. A `MenuSupplier` represents a function which takes in the id of the container and the inventory of the player viewing the menu, and returns a newly created [`AbstractContainerMenu`](https://docs.neoforged.net/docs/1.21.1/gui/menus/#abstractcontainermenu).
...
### `IContainerFactory`[​](https://docs.neoforged.net/docs/1.21.1/gui/menus/#icontainerfactory "Direct link to icontainerfactory")
...
## `AbstractContainerMenu`[​](https://docs.neoforged.net/docs/1.21.1/gui/menus/#abstractcontainermenu "Direct link to abstractcontainermenu")

All menus are extended from `AbstractContainerMenu`. A menu takes in two parameters, the [`MenuType`](https://docs.neoforged.net/docs/1.21.1/gui/menus/#menutype), which represents the type of the menu itself, and the container id, which represents the unique identifier of the menu for the current accessor.
...
Each menu implementation must implement two methods: `#stillValid` and [`#quickMoveStack`](https://docs.neoforged.net/docs/1.21.1/gui/menus/#quickmovestack).

### `#stillValid` and `ContainerLevelAccess`[​](https://docs.neoforged.net/docs/1.21.1/gui/menus/#stillvalid-and-containerlevelaccess "Direct link to stillvalid-and-containerlevelaccess")
...
### Data Synchronization[​](https://docs.neoforged.net/docs/1.21.1/gui/menus/#data-synchronization "Direct link to Data Synchronization")
...
Since `Container`s used by `Slot`s are deprecated by NeoForge in favor of using the [`IItemHandler` capability](https://docs.neoforged.net/docs/1.21.1/inventories/capabilities#neoforge-provided-capabilities), the rest of the explanation will revolve around using the capability variant: `SlotItemHandler`.
...
#### `ContainerData`[​](https://docs.neoforged.net/docs/1.21.1/gui/menus/#containerdata "Direct link to containerdata")
...
#### `#quickMoveStack`[​](https://docs.neoforged.net/docs/1.21.1/gui/menus/#quickmovestack "Direct link to quickmovestack")
...
## Opening a Menu[​](https://docs.neoforged.net/docs/1.21.1/gui/menus/#opening-a-menu "Direct link to Opening a Menu")

Once a menu type has been registered, the menu itself has been finished, and a [screen](https://docs.neoforged.net/docs/1.21.1/gui/screens) has been attached, a menu can then be opened by the player. Menus can be opened by calling `IPlayerExtension#openMenu` on the logical server. The method takes in the `MenuProvider` of the server side menu and optionally a `Consumer<RegistryFriendlyByteBuf>` if extra data needs to be synced to the client.

`IPlayerExtension#openMenu` with the `Consumer<RegistryFriendlyByteBuf>` parameter should only be used if a menu type was created using an [`IContainerFactory`](https://docs.neoforged.net/docs/1.21.1/gui/menus/#icontainerfactory).

#### `MenuProvider`[​](https://docs.neoforged.net/docs/1.21.1/gui/menus/#menuprovider "Direct link to menuprovider")

A `MenuProvider` is an interface that contains two methods: `#createMenu`, which creates the server instance of the menu, and `#getDisplayName`, which returns a component containing the title of the menu to pass to the [screen](https://docs.neoforged.net/docs/1.21.1/gui/screens). The `#createMenu` method contains three parameter: the container id of the menu, the inventory of the player who opened the menu, and the player who opened the menu.
...
### Common Implementations[​](https://docs.neoforged.net/docs/1.21.1/gui/menus/#common-implementations "Direct link to Common Implementations")

Menus are typically opened on a player interaction of some kind (e.g. when a block or entity is right-clicked).

#### Block Implementation[​](https://docs.neoforged.net/docs/1.21.1/gui/menus/#block-implementation "Direct link to Block Implementation")

Blocks typically implement a menu by overriding `BlockBehaviour#useWithoutItem`. If on the [logical client](https://docs.neoforged.net/docs/1.21.1/concepts/sides#the-logical-side), the [interaction](https://docs.neoforged.net/docs/1.21.1/items/interactionpipeline) returns `InteractionResult#SUCCESS`. Otherwise, it opens the menu and returns `InteractionResult#CONSUME`.
...
#### Mob Implementation[​](https://docs.neoforged.net/docs/1.21.1/gui/menus/#mob-implementation "Direct link to Mob Implementation")
...
[](https://docs.neoforged.net/docs/1.21.1/datastorage/saveddata)[](https://docs.neoforged.net/docs/1.21.1/gui/screens)

*   [`MenuType`](https://docs.neoforged.net/docs/1.21.1/gui/menus/#menutype)
    *   [`MenuSupplier`](https://docs.neoforged.net/docs/1.21.1/gui/menus/#menusupplier)
    *   [`IContainerFactory`](https://docs.neoforged.net/docs/1.21.1/gui/menus/#icontainerfactory)

*   [`AbstractContainerMenu`](https://docs.neoforged.net/docs/1.21.1/gui/menus/#abstractcontainermenu)
    *   [`#stillValid` and `ContainerLevelAccess`](https://docs.neoforged.net/docs/1.21.1/gui/menus/#stillvalid-and-containerlevelaccess)
    *   [Data Synchronization](https://docs.neoforged.net/docs/1.21.1/gui/menus/#data-synchronization)

*   [Opening a Menu](https://docs.neoforged.net/docs/1.21.1/gui/menus/#opening-a-menu)
    *   [Common Implementations](https://docs.neoforged.net/docs/1.21.1/gui/menus/#common-implementations)