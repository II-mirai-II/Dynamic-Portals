Title: Sides | NeoForged docs

URL Source: https://docs.neoforged.net/docs/1.21.1/concepts/sides
...
## Logical vs. Physical Side[​](https://docs.neoforged.net/docs/1.21.1/concepts/sides#logical-vs-physical-side "Direct link to Logical vs. Physical Side")

### The Physical Side[​](https://docs.neoforged.net/docs/1.21.1/concepts/sides#the-physical-side "Direct link to The Physical Side")
...
### The Logical Side[​](https://docs.neoforged.net/docs/1.21.1/concepts/sides#the-logical-side "Direct link to The Logical Side")
...
### What's the Difference?[​](https://docs.neoforged.net/docs/1.21.1/concepts/sides#whats-the-difference "Direct link to What's the Difference?")
...
If you need to transfer data from one side to another, you must [send a packet](https://docs.neoforged.net/docs/1.21.1/networking/).
...
## Performing Side-Specific Operations[​](https://docs.neoforged.net/docs/1.21.1/concepts/sides#performing-side-specific-operations "Direct link to Performing Side-Specific Operations")

### `Level#isClientSide()`[​](https://docs.neoforged.net/docs/1.21.1/concepts/sides#levelisclientside "Direct link to levelisclientside")
...
### `FMLEnvironment.dist`[​](https://docs.neoforged.net/docs/1.21.1/concepts/sides#fmlenvironmentdist "Direct link to fmlenvironmentdist")
...
#### `@Mod`[​](https://docs.neoforged.net/docs/1.21.1/concepts/sides#mod "Direct link to mod")

Checking the physical environment is important when dealing with client-only classes. The recommended way to separate code that should only be executed on one physical client is by specifying a separate [`@Mod` annotation](https://docs.neoforged.net/docs/1.21.1/gettingstarted/modfiles#javafml-and-mod), setting the `dist` parameter to the physical side the mod class should be loaded on:

`@Mod("examplemod")public class ExampleMod {    public ExampleMod(IEventBus modBus) {        // Perform logic in that should be executed on both sides    }}@Mod(value = "examplemod", dist = Dist.CLIENT) public class ExampleModClient {    public ExampleModClient(IEventBus modBus) {        // Perform logic in that should only be executed on the physical client        Minecraft.getInstance().whatever();    }}@Mod(value = "examplemod", dist = Dist.DEDICATED_SERVER) public class ExampleModDedicatedServer {    public ExampleModDedicatedServer(IEventBus modBus) {        // Perform logic in that should only be executed on the physical server    }}`
